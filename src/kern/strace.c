...
{
  snprintf (snBuf, snLg, "sys_start (%p:\"%s\", 0x%zx, %zu) = %d", name, name, entry, param, ret);
}
...
void sys_mmap_save (char *snBuf, int snLg, int fd, size_t address, size_t length, size_t offset, int flags, size_t ret)
{
  ...
  snprintf (snBuf, snLg, "sys_mmap (%d:%s, 0x%zx, 0x%zx, 0x%zx, %x) = 0x%zx", fd, ino, address, length, offset, flags, ret);
}
