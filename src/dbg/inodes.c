...
int main ()
{
  ...
  kFs_MkNode("sys", kFs_RootInode(), &rDir);
  kFs_MkNode("sys", kFs_RootInode(), &rDir);

  kFs_MkNode("bin", kFs_RootInode(), &rDir);
  kFs_MkNode("lib", kFs_RootInode(), &rDir);
  kFs_MkNode("usr", kFs_RootInode(), &rDir);
  ...
  kFs_LookFor ("cdrom:\\BOOT\\GRUB\\I386_PC/FAT.MOD", NULL);
  kFs_LookFor ("cdrom:/BOOT/GRUB/I386_PC/FIT.MOD", NULL);

  kFs_Open (kimg);

  kFs_ReadLink (kimg, buffer, 2048);
  kprintf ("Found - %s \n", buffer);

  kFs_ReadUri (kimg, buffer, 2048);
  kprintf ("Found - %s \n", buffer);
